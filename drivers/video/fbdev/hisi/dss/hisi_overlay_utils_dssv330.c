	...
	dss_layer_t *layer, dss_rect_t *wb_dst_rect, dss_wb_layer_t *wb_layer)
{
	...
	if (g_fpga_flag == 0) {
		if ((pov_req->ovl_idx == DSS_OVL2) || (pov_req->ovl_idx == DSS_OVL3)) {
			...
			if (wb_layer != NULL) {
				aif->aif_ch_ctl = set_bits32(aif->aif_ch_ctl, 0x1, 3, 8);   //credit step lower mode
				aif->aif_ch_ls = set_bits32(aif->aif_ch_ls, 0x40081, 32, 0); ///* credit en lower
			}
		}
	} else {
		//for FPGA
	...
	if (g_fpga_flag == 0) {
		if ((ovl_idx == DSS_OVL2) || (ovl_idx == DSS_OVL3)) {
			if (layer && (layer->need_cap & CAP_AFBCD)) {
				...
				aif1->aif_ch_ls = set_bits32(aif1->aif_ch_ls, 0x40081, 32, 0); ///* credit en lower
			}
		}
	} else {
		if ((ovl_idx == DSS_OVL2) || (ovl_idx == DSS_OVL3)) {
	...
	dss_layer_t *layer, dss_wb_layer_t *wb_layer)
{
	...
	/* config rch csc */
	if (layer != NULL && dpufd->dss_module.csc_used[chn_idx]) {
		...
		csc->idc0 = set_bits32(csc->idc0,
			(uint32_t)csc_coe_yuv2rgb[2][3] |
			((uint32_t)csc_coe_yuv2rgb[1][3] << 16), 27, 0);
		csc->idc2 = set_bits32(csc->idc2,
			(uint32_t)csc_coe_yuv2rgb[0][3], 11, 0);

		csc->odc0 = set_bits32(csc->odc0,
			(uint32_t)csc_coe_yuv2rgb[2][4] |
			((uint32_t)csc_coe_yuv2rgb[1][4] << 16), 27, 0);
		csc->odc2 = set_bits32(csc->odc2,
			(uint32_t)csc_coe_yuv2rgb[0][4], 11, 0);

		csc->p0 = set_bits32(csc->p0, (uint32_t)csc_coe_yuv2rgb[0][0], 13, 0);
		csc->p0 = set_bits32(csc->p0, (uint32_t)csc_coe_yuv2rgb[0][1], 13, 16);

		csc->p1 = set_bits32(csc->p1, (uint32_t)csc_coe_yuv2rgb[0][2], 13, 0);
		csc->p1 = set_bits32(csc->p1, (uint32_t)csc_coe_yuv2rgb[1][0], 13, 16);

		csc->p2 = set_bits32(csc->p2, (uint32_t)csc_coe_yuv2rgb[1][1], 13, 0);
		csc->p2 = set_bits32(csc->p2, (uint32_t)csc_coe_yuv2rgb[1][2], 13, 16);

		csc->p3 = set_bits32(csc->p3, (uint32_t)csc_coe_yuv2rgb[2][0], 13, 0);
		csc->p3 = set_bits32(csc->p3, (uint32_t)csc_coe_yuv2rgb[2][1], 13, 16);

		csc->p4 = set_bits32(csc->p4, (uint32_t)csc_coe_yuv2rgb[2][2], 13, 0);
	}

	/* config rch pcsc */
	if (layer != NULL && dpufd->dss_module.pcsc_used[chn_idx]) {
		csc = &(dpufd->dss_module.pcsc[chn_idx]);
		csc->mprec = CSC_MPREC_MODE_RGB2YUV;
		csc->icg_module = set_bits32(csc->icg_module, 0x1, 1, 0);

		csc->idc0 = set_bits32(csc->idc0,
			(uint32_t)csc_coe_rgb2yuv[2][3] |
			((uint32_t)csc_coe_rgb2yuv[1][3] << 16), 27, 0);
		csc->idc2 = set_bits32(csc->idc2,
			(uint32_t)csc_coe_rgb2yuv[0][3], 11, 0);

		csc->odc0 = set_bits32(csc->odc0,
			(uint32_t)csc_coe_rgb2yuv[2][4] |
			((uint32_t)csc_coe_rgb2yuv[1][4] << 16), 27, 0);
		csc->odc2 = set_bits32(csc->odc2,
			(uint32_t)csc_coe_rgb2yuv[0][4], 11, 0);

		csc->p0 = set_bits32(csc->p0, (uint32_t)csc_coe_rgb2yuv[0][0], 13, 0);
		csc->p0 = set_bits32(csc->p0, (uint32_t)csc_coe_rgb2yuv[0][1], 13, 16);

		csc->p1 = set_bits32(csc->p1, (uint32_t)csc_coe_rgb2yuv[0][2], 13, 0);
		csc->p1 = set_bits32(csc->p1, (uint32_t)csc_coe_rgb2yuv[1][0], 13, 16);

		csc->p2 = set_bits32(csc->p2, (uint32_t)csc_coe_rgb2yuv[1][1], 13, 0);
		csc->p2 = set_bits32(csc->p2, (uint32_t)csc_coe_rgb2yuv[1][2], 13, 16);

		csc->p3 = set_bits32(csc->p3, (uint32_t)csc_coe_rgb2yuv[2][0], 13, 0);
		csc->p3 = set_bits32(csc->p3, (uint32_t)csc_coe_rgb2yuv[2][1], 13, 16);

		csc->p4 = set_bits32(csc->p4, (uint32_t)csc_coe_rgb2yuv[2][2], 13, 0);
	}
...
void dpufb_dss_disreset(struct dpu_fb_data_type *dpufd)
{
	...
	char __iomem *pctrl_base = dpufd->pctrl_base;
	uint32_t ret;
